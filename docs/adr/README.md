...
| [ADR-0093](./ADR-0093-multi-location-bulk-and-one-slot-one-type.md) | Multi-location **bulk** entry (`POST /{commission-rates,rates}/bulk`, SAVEPOINT-per-row, EXISTS=skip-never-overwrite) + the **one-slot-one-type** rule (commission `(user,location)`; rate `(client,product,unit,location)`, app-layer on new-saves+reactivation) + the merged single+multi CREATE_PAGE_STANDARD entry page. **Additive: no schema/resolution change, no migration** — the `*_no_overlap` EXCLUDEs already admit N distinct-location rows. Extends ADR-0016/0018/0036/0048/0050/0068/0071 | Accepted · Owner-directed (commission 2026-07-11, rate 2026-07-12) | 2026-07-12 |

| [ADR-0094](./ADR-0094-scope-read-vs-write-split.md) | **PROPOSED — not implemented.** Split the two scope authorities: CLIENT/PRODUCT grants cap **WRITE** (case-create pickers + `POST /cases`, unchanged, fail-closed); **hierarchy** governs **READ** (a leader monitors their own team's work on clients they may not create on). Raised by the 2026-07-14 RBAC audit: RESTRICT caps regardless of hierarchy, so a TEAM_LEADER is blind to her own report's cases unless separately granted that client — and granting it also grants create. Supersedes ADR-0072 **only** in how the legs compose into READs. Deliberate widening → needs CTO + Security/Compliance sign-off + 4 hardening prerequisites first | **Proposed** · awaiting CTO + Security/Compliance owner | 2026-07-14 |
