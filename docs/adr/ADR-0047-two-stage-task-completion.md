...
- **Date:** 2026-06-18
- **Coordinates with:** [ADR-0046](./ADR-0046-commission-location-and-tat-dimensions.md) (commission
  resolution dimensions) — both edit `apps/api/src/platform/billing/laterals.ts` + the billing/tasks
  read-models; the commission-gate change here must merge with ADR-0046's lateral rework.
...
4. **Billing splits across the two stages, using existing machinery:**
   - **Field commission** (what we pay the field executive, `commission_rates` keyed on `assigned_to`)
     fires on **`SUBMITTED`** — the read-model commission gate moves from `status = 'COMPLETED'` to
     `status IN ('SUBMITTED','COMPLETED')`, dated by `submitted_at`.
   - **Client billing** (the bill amount via rate-management `rates`, keyed client + product + VU +
     location) fires on **`COMPLETED`** — **unchanged**. There is **no office-user payout**; the "FE"
     side is the client charge, which rate management already resolves.
