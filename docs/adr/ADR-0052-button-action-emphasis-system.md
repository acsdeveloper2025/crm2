...
- ⏳ **Migration PENDING:** point the ~120 call-sites at `<Button>` (Edit/Export/Import→secondary tonal w/ ↓/↑ icons, Delete/Deactivate→destructive, the 21 text-links→buttons) + add icon/loading props + Playwright e2e. Subagent-driven (big mechanical pass).
