...
**Process:** isolated worktree off latest `origin/main`; strict TDD (vitest RED→GREEN; no jsdom → component DOM via Playwright e2e); per-task gate = typecheck + eslint + prettier + web tests + web build green before each commit; conventional commits, no AI trailer, no `--no-verify`; nothing pushed/merged without owner OK; build per ADR-0051/0052.
- ✅ **F7 core BUILT** (`12b11f1`): `components/ui/Button.tsx` (`Button` + pure `buttonClass`) — primary/secondary-tonal/destructive/ghost/link + sizes; secondary tonal text = `text-primary-hover` (WCAG-AA in light AND dark; dark `--primary` was only ~3.9:1). 7 unit tests.
- ✅ **F7 extended** (`23f2ad2`): `loading` (inline hexagon marcher — never a spinning circle, per loading standards), `iconOnly` (square ≥44px, `aria-label` TS-required), and `components/ui/icons.tsx` Download/Upload glyphs so same-colour Export/Import differ by icon. `buttonClass` gained an `iconOnly` option (unit-tested, 9 total).
- ✅ **MIGRATION DONE** (`c61658c`…`a69cb8c`, 8 commits, subagent-driven across 6 disjoint page-clusters): all ~130 `.btn`/`.btn-ghost` + 21 text-link call-sites across 38 files now route through `<Button>` — Edit/Export/Import/Activate/Revise → secondary tonal (Export ↓ / Import ↑ glyphs), Delete/Deactivate/Revoke → destructive, Cancel/Views/Columns/pager/More → ghost, Create/Save/Submit → primary (loading). `<label>`/file-pickers use `buttonClass()` (stay `<label>`). Form submits kept `type="submit"`; RBAC gates preserved. Orphaned `.btn`/`.btn-ghost` CSS retired from `index.css` so nothing can bypass the contract. Browser-verified light + dark (admin master-data, DataGrid toolbar, dialogs). Full FE gate green (typecheck/eslint/prettier/47 tests/build).
- ⏳ **REMAINING:** a Playwright/axe e2e asserting variant rendering + the icon-only `aria-label` at the DOM level (no jsdom → e2e only); rule-of-one-primary review sweep.
