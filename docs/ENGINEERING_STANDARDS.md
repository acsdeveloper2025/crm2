...
7. **Input/Display Casing Rule** (ADR-0058, 2026-06-22 — supersedes the prior display-only rule) — user-entered display text is auto-uppercased in **both** display AND storage. Web typing goes through the shared `<Input>`/`<TextArea>` components (`apps/web/src/components/ui/`, predicate in `apps/web/src/lib/uppercase.ts`); the server safety net is `@crm2/sdk` `toUpper` (`packages/sdk/src/text.ts`) appended to display-text fields in the create/update schemas. CSS (`@crm2/ui-theme/tokens.css`) uppercases inputs/textareas too. **Case-sensitive safe set stays verbatim** (preserved in BOTH layers): case-sensitive input types (email/password/url/tel/number/date/…), name/id tokens (username, email, password, url, token/jwt/secret/apikey, otp, pin, mfa, path), regex-validated UPPER_SNAKE codes, enums, IDs, phone numbers, and content/template/JSON blobs. Auth (username/password) untouched; mobile verbatim jsonb (ADR-0054) untouched. Render case-sensitive data in `font-mono` or `.case-sensitive`. Use the components, not ad-hoc `.toUpperCase()`. Full spec: [ADR-0058](adr/ADR-0058-input-uppercase-store-not-just-display.md); `UPPERCASE_DISPLAY_STANDARD.md` is superseded.
