...
**Status: ✅ SHIPPED + LIVE 2026-07-07** — prod RDS encrypted at rest (`crm2-prod-enc`, KMS `aws/rds`), prod S3 on Intelligent-Tiering. AWS-side only; no deploy, no app change. SoT: [ADR-0091](adr/ADR-0091-rds-storage-encryption.md). Owner follow-up CLOSED same day — old instance + all unencrypted snapshots of it deleted (pre-production call); remaining unencrypted artifact = manual `crm2-pre-cleanup-20260704` (pre-cleanup test-data state, kept unless owner says drop).
