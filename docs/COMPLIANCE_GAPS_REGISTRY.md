...
- **✅ FIXED — Batch 3 shipped (UX-1 + UX-2, ADR-0092)** (branch `feat/client-setup-hub`, commits `32d7675..8ec2616`, built 2026-07-08; 15 tasks, each TDD + reviewer-gated [7 fix loops, 2 opus-reviewed write paths]; per-slice `pnpm verify` exit 0 ×6; browser/live-API verified per slice incl. a full workbook round-trip on crm2_dev [6 `import_log` rows proven]; Playwright journeys A+B; final 3-lens whole-branch review **MERGE_READY ×3** [CTO/arch · Design/governance · Security/RBAC] + accepted-fix cluster `8ec2616`):
