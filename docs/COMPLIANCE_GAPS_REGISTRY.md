...
>
> **UPDATE 2026-06-24 (supersedes the original "audit-only / none applied" framing):** the crm2-side findings were **fixed + shipped to prod** this day — report-render cluster (mig 0088) + `A2026-0623-05/06/07/09/10/16/17` ✅ FIXED and `04` split (KYC ✅ FIXED, FIELD WONTFIX/by-design), via **[ADR-0061]**(adr/ADR-0061-kyc-scoped-by-assignment-not-territory.md) + **[ADR-0062]**(adr/ADR-0062-verification-routing-and-evidence-integrity.md) and migrations `0088`/`0089`/`0090`. **See each finding's own line for its current ✅ FIXED / WONTFIX disposition — the per-finding lines are the source of truth.** Remaining **open = the mobile repo only** (`A2026-0623-02` outcome-sync, `A2026-0623-11` masked revoke — `crm-mobile-native`, separate gate, owner-release-gated).
