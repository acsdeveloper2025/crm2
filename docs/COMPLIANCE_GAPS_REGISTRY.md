...
## Section H — Frontend design-compliance audit (2026-06-19)

**Authorized multi-agent audit of `apps/web` vs the v2 design-system + engineering-standard contract** (freeze waived by owner for this run; AUDIT-ONLY, no code changed). Full deliverable: [`docs/design-audit-2026-06-19/`](./design-audit-2026-06-19/README.md) (README · CHECKLIST · MATRIX · FINDINGS · PER_PAGE · REMEDIATION_PLAN · VISUAL_VERIFICATION). Method: 1 checklist agent + 22 page-audit agents (all 13 dims, file:line) + 4 whole-app sweeps (tokens/tables/a11y/reuse) + an adversarial skeptic pass. **Audited rev:** `origin/main` `11997a1`. **Status: COMPLETE** — the first pass hit the account token cap (killing 3 sweeps + the skeptic mid-run); they were re-run successfully after the 8pm IST reset. **Skeptic:** 17 load-bearing verdicts re-verified at source, no false-PASS/false-FAIL; 2 calibration refinements folded in below.

**Result:** 286 cells → 131 PASS · 88 PARTIAL · 34 FAIL · 32 N/A · 1 UNVERIFIED. **126 findings: 0 P0 · 25 P1 · 56 P2 · 45 P3.** No P0 (nothing broken/inaccessible); gaps are *consistency of primitive adoption* on newer/bespoke surfaces. The Admin CRUD cluster + Universal DataGrid are the reference implementations; every fix is additive.

| ID | Pattern (→ REMEDIATION R#) | Sev | Disposition |
|---|---|---|---|
| H-1 | **RBAC-UI client-gating leaks** — write controls shown without `useAuth().has(perm)` (R1) | P1: **Cases `+New`** + **Rate Mgmt** (seeded TL/BE/MGR roles reach them: `case.view`≠`case.create`, `page.masterdata`≠`masterdata.manage`). P2-latent: **Templates, RBAC Roles, Policies** (skeptic: `page.templates` etc. seeded to SUPER_ADMIN only → reachable only via a custom runtime role) | 🟡 **DEFERRED → priority** (net-new). Fix all (cheap, same `useAuth().has()` pattern); Cases+Rate Mgmt first. Pending owner approval of Wave 1. |
| H-2 | **Bespoke dialogs/popovers not focus-trapped** — skip `useFocusTrap`/menu semantics (Commission Rates, RBAC Roles, Security, App-Shell Jobs/Bell/Account) (R2) | 5× P1 | 🟡 DEFERRED → Wave 1. Partial overlap C-9. |
| H-3 | **Non-standard loading/error states** — bespoke `…`/`Loading…`, no error+Retry, errors coerced to empty (Dashboard, CPV, Policies, Security, NotificationBell, Case detail/create) (R3) | 4× P1 | 🟡 DEFERRED → Wave 1. Overlaps B-7/B-8. |
| H-4 | **Forms bypass RHF+`zodResolver(@crm2/sdk)`; OCC 409 as inline string not `ConflictDialog`** (Commission Rates, Policies, Report Layouts, admin dialogs) (R4) | 4× P1 | 🟡 DEFERRED → fold into **C-10** (OCC retrofit). |
| H-5 | **Embedded sub-lists are bespoke `<table>`** not DataGrid (R5). **Refined by tables-sweep + skeptic:** DataGrid backs all 19 *paginated list* pages; bespoke tables are confined to detail/child/modal/rollup surfaces — mostly **defensible** (pagination overkill). Billing case-lines is inside the sanctioned `renderExpanded` slot (NOT a violation). Genuine flags: Case detail's 3 tables (busiest bespoke page), CPV enabled-units (a growing list), Billing standalone breakdown panels — adopt the `.rtable`+`data-label`+focusable-scroll-region pattern, or DataGrid where the list can grow. | P2/P3 | 🟡 DEFERRED → fold into **C-9** (responsive retrofit). |
| H-6 | **Export/pagination contract bypass** — Users `Export Scope` skips 413/job; Case detail unbounded arrays; Policies no export (R6) | 2× P1 | 🟡 DEFERRED → fold into **B-13** (universal export). |
| H-7 | **Token slips** — `text-st-completed` (non-existent token → unstyled, `CaseCreatePage.tsx:257`) + `text-amber-600` (raw palette, `AddTasksForm.tsx:173`) (R7) | 1× P1 | 🟡 **DEFERRED → trivial Wave 1** (net-new; 2 one-line edits + a lint rule for unknown `st-*` names). |
| H-8 | **Status/money/date not via shared fmt** (status as plain text not `StatusChip` on Cases/Case-detail; inline ₹/date) + consistency (Users duplicate `Import`; Locations bespoke create form; `.toUpperCase()` in `onChange`) + ~44px touch targets (R8/R9/R10) | P2/P3 | 🟡 DEFERRED → Wave 3 polish. Status-as-text is net-new. |
| H-9 | **Add/Edit interaction inconsistency** (D14, follow-up audit 2026-06-20 — [ADD_EDIT_PATTERN.md](./design-audit-2026-06-19/ADD_EDIT_PATTERN.md)) — **11 pages use popup modals for add+edit, 3 are split**. Skeptic confirmed all classifications (0 overturned). | n/a (standardization) | 🟢 **DECISION MADE → [ADR-0051](./adr/ADR-0051-inline-grid-editing-no-modal-forms.md) ACCEPTED (owner 2026-06-20):** Twenty-style **inline-grid cell editing** for flat entities + **full record-page routes** for complex ones (Users/Roles/ReportLayouts/CommissionRates); **delete all add/edit `*Dialog` modals**; OCC/import/confirm/menus stay. Build = **Wave 4** (editable-DataGrid Foundation is the big item; also delivers the Wave-K DataGrid keyboard P1s). Supersedes dialog-CRUD in ADR-0008 / `MANAGEMENT_LIST_STANDARD`. Awaiting owner go to BUILD. |
| H-10 | **Keyboard navigation** (D15, follow-up audit 2026-06-20 — [KEYBOARD_NAV.md](./design-audit-2026-06-19/KEYBOARD_NAV.md)) — **5 P1 blockers:** DataGrid sortable headers + onRowClick rows keyboard-unreachable (every grid), CommissionRates dialog no trap, RBAC "cannot deactivate" overlay no trap, RateMgmt `SearchableSelect` mouse-only select. Plus P2/P3: no skip-to-content, header popovers no trap, MustAcceptPolicies scroll region, `.input` focus-ring suppressed. **Skeptic refinement:** a global `:focus-visible` ring DOES exist (`tokens.css:238-240`) — the focus-ring defect is text-`.input`-only (P2), NOT an app-wide P1 (that claim was overstated). | 5× P1 + P2/P3 | 🟡 DEFERRED → Wave K. DataGrid header/row keyboard = Foundation (fixes all grids at once). |
| H-11 | **Button affordance / action-emphasis** (owner-reported 2026-06-20) — only 2 button styles (`.btn` filled-blue + `.btn-ghost` thin-grey border, `index.css:14-19`) + **21 files use bare `text-primary` link-style buttons** for real row actions (Edit/Deactivate). With one blue + grey/text everywhere, **users can't tell secondary/row controls are buttons**. No secondary/destructive/icon-button variants; no shared `<Button>`; inconsistent sizes. `secondary`/`destructive` tokens already exist (`tailwind-preset.js:33-43`) — fix is additive. | P2 (UX) | 🟢 **DECISION MADE → [ADR-0052](./adr/ADR-0052-button-action-emphasis-system.md) ACCEPTED (owner 2026-06-20):** add a `<Button>` variant system (primary/secondary/outline/destructive/ghost/link + sizes/icon/loading), every control gets a button affordance (bg or border + hover + focus), one primary per view, row actions → `secondary`/`outline`, deletes → `destructive`. Extends ADR-0008/COLOR_SYSTEM_FREEZE additively (no new colors). **Locked mapping (owner-approved 2026-06-20, from the [button inventory](./design-audit-2026-06-19/BUTTON_INVENTORY.md) of ~200 buttons):** primary=blue (Create/Save), **secondary=bordered (Edit/Export/Import/Activate)**, **destructive=filled red (Deactivate/Delete/Revoke)**, ghost=utility/Cancel. Real gaps were Edit (11 text-links) + Export (4 styles). Build = plan **Foundation F7** + migration (supersedes Wave-3 C3 button item). |

**Recommendation:** approve **Wave 1 (H-1, H-2, H-3, H-7)** as one small PR — removes every P1 except the table/contract items at near-zero blast radius. Wave 2 (H-4/H-5/H-6) folds into existing C-9/C-10/B-13. Full per-finding evidence + fixes: [FINDINGS.md](./design-audit-2026-06-19/FINDINGS.md).

**Multi-agent fix plan (2026-06-20, awaiting owner approval):** [docs/plans/2026-06-20-frontend-design-compliance-fix-plan.md](./plans/2026-06-20-frontend-design-compliance-fix-plan.md) — Foundation (build shared primitives: centralized `has()`, `formatMoney`, `WorkStatusChip`, shared `Popover`, token guard, **`<Button>` variant system [H-11/ADR-0052]**) done serially first, then **Wave 1–3** (P1 → contract → polish), **Wave 4** (H-9 inline-grid add/edit — ADR-0051), **Wave K** (H-10 keyboard nav; DataGrid header/row keyboard is Foundation), fanned out across parallel agents on disjoint files, TDD + `pnpm verify` + axe-gate per task. Dispositions flip FIXED here as each wave lands.
