...
- **A2026-0623-04** [HIGH/report] (BUSINESS) — FIELD_AGENT EXPAND (PINCODE/AREA) broadens case.view to other agents' case PII — case-view path lacks the assigned_to=self hard filter that… _(src: BUSINESS/SEC-BUS-1(S))_ → **FIX — re-verified REAL, OWNER DECISION PENDING (2026-06-24).** Confirmed (Explore agent): FIELD_AGENT has `case.view` + PINCODE/AREA **EXPAND** (`0033_roles.sql:74`, `0034_scope_dimensions.sql:60-61`); `cases/repository.ts` `caseScopePredicate` OR's the EXPAND territory legs at the root, so a field agent sees other agents' cases (full applicant PII) in their pincode/area — the `assigned_to=self` AND-gate that sync has (`sync/repository.ts:98`) is absent. NOT auto-fixed: the fix touches shared scope infra (`composeScopePredicate`, used by cases/tasks/dashboard) and/or an RBAC seed dimension — both are **frozen-design** changes needing owner sign-off (possibly an ADR). Options: (a) AND-gate `assigned_to=self` for SELF-mode roles on the cases case-view/detail path; (b) drop PINCODE/AREA EXPAND from FIELD_AGENT (migration); (c) drop case.view from FIELD_AGENT if field agents are mobile-only (sync is already task-grain). Recommend (a). Distinct from A2026-0623-09 (that was the per-task fieldReports route, fixed).
