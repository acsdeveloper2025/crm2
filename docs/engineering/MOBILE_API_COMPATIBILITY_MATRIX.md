...
| **Policy acceptance gate** (ADR-0043) | v1 had a **FE-only** Field-Exec Acknowledgement guard (no server contract) | `POST /api/v2/consents/accept {policyIds[],source}` (authenticated, no permission, idempotent) — <!-- MOBILE_API_COMPATIBILITY-01, docs/audit/19-mobile-api-compatibility.md: corrected from the superseded `/auth/accept-policies` path; verified against apps/api/src/modules/consents/routes.ts + app.ts's `/api/v2/consents` mount --> **+** `mustAcceptPolicies: boolean` / `pendingPolicies: PendingPolicy[]` on the **login response** — a **locked mobile contract**: the device blocks into its accept screen when `mustAcceptPolicies`, posts to `/consents/accept` with **`source='MOBILE'`** (the value reserved for the device), and `refresh` is rejected until accepted. Lands when the mobile client rebases `/api/mobile` → `/api/v2`. |
