...
`pnpm verify` GREEN: typecheck · lint · format · no-suppressions · boundaries · build + web (69) / sdk (201) / API (1056, 80 files) tests. Browser: global uppercase + carve-outs confirmed via DOM probe, zero console errors. **✅ SHIPPED + LIVE on prod 2026-07-03** — committed `e7e4e1c`, pushed to `main` (owner-approved); deploy `28639683915` SUCCESS (4m37s, gate → GHCR → blue-green), prod health 200.
