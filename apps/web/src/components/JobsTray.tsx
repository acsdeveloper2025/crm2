 ...
 * Inline SVG + theme tokens to match the shell (no icon/color literals). Focus-trap / outside-click /
 * Escape come from the shared <Popover>.
 */
...
export function JobsTray() {
  ...
  return (
    <Popover
      label={activeCount > 0 ? `Background jobs, ${activeCount} running` : 'Background jobs'}
      panelLabel="Background jobs"
            ...
            return (
              ...
              >
                  ...
                  <span className="flex w-full flex-col items-start gap-1">
                    ...
                    {r.capped && (
                      <span className="text-[11px] text-destructive">
                        {(r.rowCount ?? 0).toLocaleString()} of {(r.totalCount ?? 0).toLocaleString()} matched
                        rows — refine filters for the full set.
                      </span>
                    )}
                  </span>
                )}
                {j.status === 'SUCCEEDED' && j.type === 'IMPORT' && (
                  <span className="text-xs text-muted-foreground">
                    Imported {(r.successRows ?? 0).toLocaleString()} of {(r.totalRows ?? 0).toLocaleString()}{' '}
                    rows
