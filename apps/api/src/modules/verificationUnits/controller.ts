...
import { verificationUnitService as svc } from './service.js';
...
export const verificationUnitController = {
  ...
  async export(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await writeExport(res, {
        ...
        columns,
        ex,
        filenameBase: 'verification-units',
  ...
  async importTemplate(_req: Request, res: Response, next: NextFunction) {
    try {
      writeTemplate(res, await svc.importTemplate(), 'verification-units');
    } catch (e) {
      next(e);
    }
  },
