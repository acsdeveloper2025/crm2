...
import { reportLayoutService as svc } from './service.js';
...
export const reportLayoutController = {
  ...
  async byConfig(req: Request, res: Response, next: NextFunction) {
    try {
      res.json(await svc.byConfig(req.query as Record<string, unknown>));
    } catch (e) {
      next(e);
    }
  },
