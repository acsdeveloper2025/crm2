...
import { designationService as svc } from './service.js';
...
export const designationController = {
  ...
  async importTemplate(_req: Request, res: Response, next: NextFunction) {
    try {
      writeTemplate(res, await svc.importTemplate(), 'designations');
    } catch (e) {
      next(e);
    }
  },
  ...
  async import(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!Buffer.isBuffer(file) || file.length === 0)
        throw AppError.badRequest('NO_IMPORT_FILE', { reason: 'empty body' });
